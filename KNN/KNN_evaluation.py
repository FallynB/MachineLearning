...
def accuracy(train_data,train_target,test_data,test_target,k):
    ...
    prediction_output = np.mean(predictions == test_target)
...
accuracies= np.array([accuracy(train_data, train_target, test_data, test_target, k_index) for k_index in k])
...
best_accuracy = accuracy(train_data,train_target,test_data,test_target,k)
print(best_accuracy)
